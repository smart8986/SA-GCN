...
H36M_NAMES[7] = 'LKnee'
H36M_NAMES[8] = 'LFoot'
H36M_NAMES[12] = 'Spine'  # 12-14
H36M_NAMES[13] = 'Thorax'
H36M_NAMES[14] = 'Neck/Nose'
H36M_NAMES[15] = 'Head'  # 16-18
    ...
    for subj in subjects:
        ...
        for action in actions:
            ...
            dpath = os.path.join(bpath, 'S{0}'.format(
                subj), 'MyPoses/{0}D_positions'.format(dim), '{0}*.h5'.format(action))
            print(dpath)
    ...
    for subj in subjects:
        ...
        #       continue
            ...
            dpath = os.path.join(data_dir, 'S{0}'.format(
                subj), 'StackedHourglass/{0}*.h5'.format(action))
            print(dpath)
            ...
            for fname in fnames:
                ...
                if seqname.startswith(action):
                    ...
                    with h5py.File(fname, 'r') as h5f:
                        ...
                        poses_final = np.zeros(
                            [poses.shape[0], len(H36M_NAMES)*2])
...
def normalization_stats(complete_data, dim, predict_14=False):
    ...
    # data_mean = np.repeat(data_mean, complete_data.shape[1])
    # data_std = np.repeat(data_std, complete_data.shape[1])

    # Encodes which 17 (or 14) 2d-3d pairs we are predicting
    dimensions_to_ignore = []
    if dim == 2:
        dimensions_to_use = np.where(
            np.array([x != '' and x != 'Neck/Nose' and x != 'Hip' for x in H36M_NAMES]))[0]
        dimensions_to_use = np.sort(
            np.hstack((dimensions_to_use*2, dimensions_to_use*2+1)))
        dimensions_to_ignore = np.delete(
            np.arange(len(H36M_NAMES)*2), dimensions_to_use)
    else:  # dim == 3
        dimensions_to_use = np.where(
            np.array([x != '' for x in H36M_NAMES]))[0]
        dimensions_to_use = np.delete(
            dimensions_to_use, [0, 7, 9] if predict_14 else 0)

        dimensions_to_use = np.sort(np.hstack((dimensions_to_use*3,
                                               dimensions_to_use*3+1,
                                               dimensions_to_use*3+2)))
        dimensions_to_ignore = np.delete(
            np.arange(len(H36M_NAMES)*3), dimensions_to_use)
...
def normalize_data_unnorm(data, data_mean, data_std, dim_to_use):
    """
    Normalizes a dictionary of poses
...
def unNorm(normalized_data, data_mean, data_std, dimensions_to_ignore):
    """
    Un-normalizes a matrix whose mean has been substracted and that has been divided by
    standard deviation. Some dimensions might also be missing
    ...
    meanMat = np.repeat(meanMat, T, axis=0)
    orig_data = np.multiply(orig_data, stdMat) + meanMat
    return orig_data[:, dimensions_to_use]


def fillData(normalized_data, data_mean, data_std, dimensions_to_ignore):
    T = normalized_data.shape[0]  # Batch size
    D = data_mean.shape[0]  # Dimensionality
...
def project_to_cameras(poses_set, cams, ncams=4):
    ...
    for t3dk in sorted(poses_set.keys()):
        ...
        for cam in range(ncams):
            ...
            if name != "55011271":
                continue
            pts2d, _, _, _, _, X = cameras.project_point_radial(
                np.reshape(t3d, [-1, 3]), R, T, f, c, k, p)

            pts2d = np.reshape(pts2d, [-1, len(H36M_NAMES)*2])
            sname = seqname[:-3]+"."+name+".h5"  # e.g.: Waiting 1.58860488.h5
            t2d[(subj, a, sname)] = pts2d
            c3d[(subj, a, sname)] = np.reshape(X.T, [-1, t3d.shape[1]])[:, 0:3]
            c3d[(subj, a, sname)][:, 0:2] = pts2d[:, 0:2]
...
def create_2d_data(actions, data_dir, rcams):
    ...
    train_set, c3d_tr = project_to_cameras(train_set, rcams)
    test_set, c3d_te = project_to_cameras(test_set, rcams)

    train_set = center_around_hip(train_set)
    test_set = center_around_hip(test_set)

    # Compute normalization statistics.
    complete_train = copy.deepcopy(np.vstack(train_set.values()))
    c3d_train = copy.deepcopy(np.vstack(c3d_tr.values()))
    avg_z = np.mean(c3d_train[:, 2])

    train_set = scale(train_set, c3d_tr, avg_z)
    test_set = scale(test_set, c3d_te, avg_z)
...
def create_2d_data_corrected(actions, data_dir, rcams):
    """
    Creates 2d poses by projecting 3d poses with the corresponding camera
    parameters. Also normalizes the 2d poses
    ...
    # Compute normalization statistics.
    c3d_train = copy.deepcopy(np.vstack(list(c3d_tr.values())))
    avg_z = np.mean(c3d_train[:, 2])
...
def create_2d_data_unnorm(actions, data_dir, rcams):
    """
    Creates 2d poses by projecting 3d poses with the corresponding camera
    parameters. Also normalizes the 2d poses
    ...
    complete_train = copy.deepcopy(np.vstack(train_set.values()))
    c3d_train = copy.deepcopy(np.vstack(c3d_tr.values()))
    assert complete_train.shape[0] == c3d_train.shape[0]
    avg_z = np.mean(c3d_train[:, 2])
    ...
    # Divide every dimension independently
    train_set = normalize_data_unnorm(
        train_set, data_mean, data_std, dim_to_use)
    test_set = normalize_data_unnorm(
        test_set,  data_mean, data_std, dim_to_use)

    return train_set, test_set, data_mean, data_std, dim_to_ignore, dim_to_use, avg_z, c3d_tr, c3d_te


def create_2d_data_sk2im(actions, data_dir, rcams):
    """
    Creates 2d poses by projecting 3d poses with the corresponding camera
    parameters. Also normalizes the 2d poses
    ...
    train_actions = ["Discussion", "Eating",
                     "Phoning", "Posing",
                     "Smoking", "Waiting"]

    # Load 3d data
    train_set = load_data(data_dir, TRAIN_SUBJECTS +
                          TEST_SUBJECTS, train_actions, dim=3)
    test_set = load_data(data_dir, TRAIN_SUBJECTS +
                         TEST_SUBJECTS, actions, dim=3)
    ...
    return train_set, test_set, data_mean, data_std, dim_to_ignore, dim_to_use, avg_z, c3d_tr, c3d_te, train_actions


def read_2d_predictions(actions, data_dir, rcams):
    """
    ...
    # predicted 2d pose
    train_set = load_stacked_hourglass(
        data_dir, TRAIN_SUBJECTS+TEST_SUBJECTS, actions)
    test_set = load_stacked_hourglass(
        data_dir, TRAIN_SUBJECTS+TEST_SUBJECTS, actions)

    train_set = center_around_hip(train_set)
    test_set = center_around_hip(test_set)

    train_set = scale(train_set, c3d_te, avg_z)
    test_set = scale(test_set, c3d_te, avg_z)
...
def center_around_hip(data):
    data_out = {}
    for key in data.keys():
        hip = data[key][:, :2]
        data_out[key] = data[key] - np.concatenate([hip]*32, axis=1)

    return data_out


def scale(data, c3d, avg_z):
    data_out = {}
    for key in data.keys():
        s = c3d[key][:, 2] / avg_z
        data_out[key] = np.multiply(data[key], np.stack([s]*64, axis=1))
