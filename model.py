...
class Generator(nn.Module):
    ...
    def forward(self, label):
        ...
        noise = torch.randn(label.size(0), self.opt.seq_length,
                            self.opt.d_E).to(self.opt.device)
        ...
        final = []
        sum = torch.rand(label.size(0), self.hidden_size).to(
        ...
        x1 = self.linear1(final.transpose(1, 2)) + \
            self.block1(final.transpose(1, 2))
    ...
    def norm(self, attn):
        ...
        A_self = torch.eye(self.opt.total_joint, self.opt.total_joint).repeat(
            attn.size()[0], 1, 1).to(attn.device)
...
class Self_Attn_Seq(nn.Module):
    def __init__(self, in_dim, opt):
        ...
        self.w_q = nn.Linear(input_dim, self.n_head *
                             self.hidden_size_attention)
        self.w_k = nn.Linear(input_dim, self.n_head *
                             self.hidden_size_attention)
        self.w_v = nn.Linear(input_dim, self.n_head *
                             self.hidden_size_attention)
        nn.init.normal_(self.w_q.weight, mean=0, std=np.sqrt(
            2.0 / (input_dim + self.hidden_size_attention)))
        ...
        self.linear2 = nn.Linear(
            self.n_head * self.hidden_size_attention, input_dim)
    ...
    def forward(self, q):
        ...
        q = q.permute(2, 0, 1, 3).contiguous().view(-1,
                                                    len, self.hidden_size_attention)
